    ...
    public class GameHub : Hub
    {
        ...
        public override async Task OnDisconnectedAsync(Exception? exception)
        {
            ...
            if (_activeGames.TryRemove(Context.ConnectionId, out var game))
            {
                ...
                await Clients.Group(game.GameId).SendAsync("ReceiveMessage", $"Player {player?.Name} has left the game.");
                // Notify the other player that the game has ended due to disconnection
                await Clients.Group(game.GameId).SendAsync("GameEnded", "Game ended due to player disconnection.");
            }
